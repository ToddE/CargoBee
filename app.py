...
def home():
    ...
    if request.method == 'POST':
        try:
            ...
            if shipment_type == 'floor_loaded':
                ...
                while sum(pallets_left_to_ship.values()) > 0:
                    ...
                    for key, container in reversed(list(CONTAINERS.items())):
                        ...
                        # Count how many pallets of each type were packed in this container
                        manifest = Counter(pallets_that_fit)
                        if sum(manifest.values()) == sum(pallets_left_to_ship.values()):
                            best_fit_container = key
                            container_manifests.append({'key': best_fit_container, 'manifest': dict(manifest)})
                            pallets_left_to_ship = {ptype: pallets_left_to_ship[ptype] - manifest.get(ptype, 0) for ptype in pallets_left_to_ship}
                            # Remove any pallet types with zero left
                            pallets_left_to_ship = {ptype: count for ptype, count in pallets_left_to_ship.items() if count > 0}
                            break
                    if not best_fit_container:
                        largest_key = list(CONTAINERS.keys())[0]
                ...
                for i, c in enumerate(container_manifests):
                    ...
                    for p_type, count in sorted(c['manifest'].items(), key=lambda item: pallet_configs[item[0]]['height'], reverse=True):
                        ...
                        container_weight += count * (conf['cartons'] * carton_weight + conf.get('pallet_weight', 0))
                        desc = f"{count} pallet(s) with {conf['layers']} layers ({conf['height']} cm) holding {conf['cartons']} cartons each, pallet weight: {conf.get('pallet_weight', 0)} kg each"
            ...
            # Global Weight Check
            total_cargo_weight = total_cartons * carton_weight + sum(total_pallet_inventory.values()) * pallet_weight
